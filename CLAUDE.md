...
> **Tascal** — 膨大なマルチタスクを「一人で捌き切る」ための「コミットメント（約束）を可視化し、AI と協働して守りきる」タスク管理ツール。

4 本柱：①一人で捌き切ることに特化 ②マルチタスク ③コミットメントの可視化 ④AI エージェントとの能動的協働。
**視点は個人（一人）に固定**。他者共有・担当者割り当て・権限（マルチユーザー管理）は**非目標**（[ADR-0003](./docs/adr/0003-single-person-focus.md)）。
...
- **非目標化（2026-06-20）**：他者共有・担当者割り当て・権限（マルチユーザー管理）は廃止（[ADR-0003](./docs/adr/0003-single-person-focus.md)）。
  M2 は「データ基盤・**端末間同期（単独ユーザー）**」へ再定義。単独同期・ホスト型 AI（M3）は「Web/同期フェーズ」を判断してから。
...
### 確定した方針（2026-06-20）
- **ビジョン再調整＝「一人で捌き切る」ことに特化**（[ADR-0003](./docs/adr/0003-single-person-focus.md)）。膨大なマルチタスク × AI コラボ × 可視化という核は**個人視点で完結**するため、視点を個人に固定。
- **マルチユーザー管理（他者共有・担当者割り当て・権限ロール）を非目標化／廃止**。Q-PROD-4 は取り下げ。Workspace/Membership は持たない。
- **M2 を「データ基盤・端末間同期（単独ユーザー）」へ再定義**。本人アカウント＋自分のデータの端末間同期のみ（共有・割り当て・権限は削除）。実装時期は Q-ARCH-2 に従属。
- **保持**：①`stakeholder`（約束の相手＝外部の宛先）は自由記述の任意項目として維持 ②**AI への委任（owner=AI / `ownerType:'ai'`）は将来必要**として柱④・北極星（M7）で扱う（人間同士の割り当てだけ廃止）。
